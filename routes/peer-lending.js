...
router.post("/", async (req, res) => {
  const { accessToken, commitedAmount, interestRate } = req.body

  if (!accessToken)
    return res.json({ status: 401, message: "Unauthorized access." })

  if (!commitedAmount || commitedAmount < 250)
  ...
  try {
    await createListing({ userId: accessToken.userID, commitedAmount, interestRate })
...
async function createListing (record) {
  const { userId, commitedAmount, interestRate } = record

  return mySql.handleQuery(`
    INSERT INTO peer_listing (commited_amount, interest_rate, listed_by)
    VALUES
    (?, ?, ?)
  `, [commitedAmount, interestRate, userId])
}
